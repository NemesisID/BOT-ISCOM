...
SESSION_COOKIE = "iscom_surface_session"
...
async def _ensure_guild_records(guild_id: int, bot_guild) -> dict[str, Any]:
    ...
    ai_config = cache.ai_settings.get(str(guild_id)) or await storage.ai_settings.insert(
        guild_id=guild_id,
        enabled=False,
        api_base_url="",
        api_key="",
    ...
    counters_config = cache.counters.get(str(guild_id)) or await storage.counters.insert(
        guild_id=guild_id,
        enabled=False,
        member_counter_channel_id=None,
        bot_counter_channel_id=None,
        boost_counter_channel_id=None,
    )
    ...
    return {
        ...
        "ai_settings": ai_config or cache.ai_settings.get(str(guild_id), {}),
        "counters": counters_config or cache.counters.get(str(guild_id), {}),
    }
...
def _render_login(notice: str | None = None) -> str:
    ...
    """
    return _render_layout(title="ISCOM Login", body=body, session=None, notice=notice)
...
def _render_guild_picker(session: dict[str, Any], guilds: list[dict[str, Any]], notice: str | None = None) -> str:
    ...
    """
    return _render_layout(title="ISCOM Guilds", body=body, session=session, guilds=guilds, notice=notice)
...
def _render_overview(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], bot_guild, state: dict[str, Any], notice: str | None = None) -> str:
          ...
          <div class="fields">
            ...
            <label>Language
              <select name="language">
                ...
                <option value="id" {"selected" if guild_state.get("language") == "id" else ""}>Indonesian</option>
              </select>
            </label>
          </div>
    ...
    return _render_layout(
        title=f"ISCOM Overview - {current_guild['name']}",
        body=body,
        session=session,
        guilds=guilds,
...
def _render_security(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: str | None = None) -> str:
    ...
    """
    return _render_layout(title=f"ISCOM Security - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="security", notice=notice)
...
def _render_moderation(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: str | None = None) -> str:
    ...
    """
    return _render_layout(title=f"ISCOM Moderation - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="moderation", notice=notice)
...
def _render_music(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], bot_guild, state: dict[str, Any], notice: str | None = None) -> str:
    ...
    """
    return _render_layout(title=f"ISCOM Music - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="music", notice=notice)
...
def _render_commands(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: str | None = None) -> str:
    ...
    """
    return _render_layout(title=f"ISCOM Commands - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="commands", notice=notice)
...
def _render_logs(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], notice: str | None = None) -> str:
    ...
    """
    return _render_layout(title=f"ISCOM Logs - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="logs", notice=notice)


def _render_ai(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: str | None = None) -> str:
    data = state["ai_settings"]
    body = f"""
    <section class="panel">
      <h1>AI Settings</h1>
      ...
      <form method="post" action="/dashboard/guild/{current_guild['id']}/ai" enctype="multipart/form-data" style="margin-top:18px;">
        ...
        <div class="fields">
          <label>API Base URL
            <input type="text" name="api_base_url" value="{_escape(data.get('api_base_url', ''))}" placeholder="https://api.openai.com/v1">
          </label>
          <label>API Key
    ...
    """
    return _render_layout(title=f"ISCOM AI - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="ai", notice=notice)


def _render_counters(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: str | None = None) -> str:
    data = state["counters"]
    body = f"""
    <section class="panel">
      <h1>Automatic Counters</h1>
      <p class="muted">Configure live member count voice channels</p>
      <form method="post" action="/dashboard/guild/{current_guild['id']}/counters" style="margin-top:18px;">
        <div class="switches">
          <label class="check"><input type="checkbox" name="enabled" {"checked" if data.get("enabled") else ""}> Enable automatic counters</label>
        </div>
        <div class="fields">
          <label>Member Counter Channel ID
            <input type="text" name="member_counter_channel_id" value="{_escape(data.get('member_counter_channel_id', ''))}" placeholder="Voice channel ID for member count">
          </label>
          <label>Bot Counter Channel ID
            <input type="text" name="bot_counter_channel_id" value="{_escape(data.get('bot_counter_channel_id', ''))}" placeholder="Voice channel ID for bot count">
          </label>
          <label>Boost Counter Channel ID
            <input type="text" name="boost_counter_channel_id" value="{_escape(data.get('boost_counter_channel_id', ''))}" placeholder="Voice channel ID for boost count">
          </label>
        </div>
        <button class="save-btn" type="submit">Save counter settings</button>
      </form>
    </section>
    """
    return _render_layout(title=f"ISCOM Counters - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="counters", notice=notice)
...
def _render_welcomer(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: str | None = None) -> str:
    ...
    """
    return _render_layout(title=f"ISCOM Welcomer - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="welcomer", notice=notice)
...
async def dashboard_tab(request: Request, guild_id: int, tab: str, notice: str | None = None):
    ...
    renderers = {
        ...
        "ai": lambda: _render_ai(session, guilds, current_guild, state, notice=notice),
        "counters": lambda: _render_counters(session, guilds, current_guild, state, notice=notice),
...
@router.post("/guild/{guild_id}/ai")
async def update_ai_settings(request: Request, guild_id: int):
    session, _, current_guild, state = await _require_dashboard_context(request, guild_id)
    if not session or not current_guild:
        return RedirectResponse("/dashboard", status_code=303)
...
@router.post("/guild/{guild_id}/counters")
async def update_counters_settings(request: Request, guild_id: int):
    session, _, current_guild, state = await _require_dashboard_context(request, guild_id)
    if not session or not current_guild:
        return RedirectResponse("/dashboard", status_code=303)
    data = await _parse_form(request)
    await storage.counters.update(
        id=state["counters"]["id"],
        enabled=_bool_from_form(data, "enabled"),
        member_counter_channel_id=int(data["member_counter_channel_id"]) if data.get("member_counter_channel_id", "").isdigit() else None,
        bot_counter_channel_id=int(data["bot_counter_channel_id"]) if data.get("bot_counter_channel_id", "").isdigit() else None,
        boost_counter_channel_id=int(data["boost_counter_channel_id"]) if data.get("boost_counter_channel_id", "").isdigit() else None,
    )
    return RedirectResponse(f"/dashboard/guild/{guild_id}/counters?notice=Counter%20settings%20saved", status_code=303)
