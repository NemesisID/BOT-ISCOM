...
from storage import ai_settings as ai_settings_db


from storage import counters as counters_db
...
class AISettings:
    ...
    def __init__(self):
        ...
        logger.database("All AI Settings cache initialized")

    async def initialize(self):

        cache.ai_settings = {}

        ai_settings_datas = await ai_settings_db.get_all()

        for data in ai_settings_datas:

            if data.get("guild_id"):

                cache.ai_settings[str(data["guild_id"])] = data

        logger.database("All AI Settings cache loaded")

    async def delete(self, guild_id):

        if str(guild_id) in cache.ai_settings:

            del cache.ai_settings[str(guild_id)]

            logger.database(f"AI Settings - {guild_id} cache deleted")

        else:

            logger.error(f"AI Settings - {guild_id} cache not found")

    async def update(self, guild_id, data=None):

        if not data:

            data = await ai_settings_db.get(guild_id=guild_id)
        ...
        cache.ai_settings[str(guild_id)] = data

        logger.database(f"AI Settings - {guild_id} cache updated")
...
class Counters:
    ...
    def __init__(self):
        ...
        logger.database("All Counters cache initialized")

    async def initialize(self):

        cache.counters = {}

        counters_datas = await counters_db.get_all()

        for data in counters_datas:

            if data.get("guild_id"):

                cache.counters[str(data["guild_id"])] = data

        logger.database("All Counters cache loaded")

    async def delete(self, guild_id):

        if str(guild_id) in cache.counters:

            del cache.counters[str(guild_id)]

            logger.database(f"Counters - {guild_id} cache deleted")

        else:

            logger.error(f"Counters - {guild_id} cache not found")

    async def update(self, guild_id, data=None):

        if not data:

            data = await counters_db.get(guild_id=guild_id)
        ...
        cache.counters[str(guild_id)] = data
