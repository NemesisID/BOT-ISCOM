...
class BaseHelpView(ui.LayoutView):
    ...
    def tr(self, english: str, indonesian: str) -> str:
        return indonesian if self.get_language() == "id" else english
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    description=self.tr(
                        ...
                        "Anda tidak diizinkan menggunakan interaksi ini",
                    ),
                    color=color.red,
                ),
                ephemeral=True,
...
class HomeView(BaseHelpView):
    def __init__(self, bot, ctx, all_app_commands, reported=False):
        ...
        container.add_item(
            ui.TextDisplay(
                self.tr("## Select a category to view", "## Pilih kategori untuk melihat")
            )
        )
...
class CategorySelectRow(ui.ActionRow["HomeView"]):
    def __init__(self, view: HomeView):
        ...
        select = ui.Select(
            placeholder=view.tr("Select a category to view", "Pilih kategori untuk melihat"),
            options=options,
        )
    ...
    async def select_category(self, interaction: discord.Interaction):
        ...
        if not cog:
            return await interaction.response.send_message(
                embed=discord.Embed(
                    description=self.view.tr("Category not found", "Kategori tidak ditemukan"),
                    color=color.red,
                ),
                ephemeral=True,
                delete_after=10,
...
class AllCommandsView(BaseHelpView):
    def __init__(self, bot, ctx, all_app_commands, reported=False):
        super().__init__(bot, ctx, all_app_commands, reported)
        container = ui.Container()
        container.add_item(ui.TextDisplay(f"# {self.tr('All Commands', 'Semua Perintah')}"))
        container.add_item(ui.Separator(spacing=discord.SeparatorSpacing.large))
        main_cogs, extra_cogs, _ = self.get_cogs()
        for cog in main_cogs + extra_cogs:
            cmds = " | ".join(
...
class CategoryView(BaseHelpView):
    def __init__(self, bot, ctx, all_app_commands, cog, reported=False):
        ...
        container.add_item(
            ui.TextDisplay(
                self.tr("## Select a command to view", "## Pilih perintah untuk melihat")
            )
        )
...
class CommandSelectRow(ui.ActionRow["CategoryView"]):
    def __init__(self, view: CategoryView):
        ...
        select = ui.Select(
            placeholder=view.tr("Select a command to view", "Pilih perintah untuk melihat"),
            options=options,
        )
    ...
    async def select_command(self, interaction: discord.Interaction):
        ...
        if not command:
            return await interaction.response.send_message(
                embed=discord.Embed(
                    description=self.view.tr("Command not found", "Perintah tidak ditemukan"),
                    color=color.red,
                ),
                ephemeral=True,
                delete_after=10,
...
class CommandView(BaseHelpView):
    def __init__(self, bot, ctx, all_app_commands, command, cog, reported=False):
        ...
        self.cog = cog
        container = ui.Container()
        desc = f"# {self.command.name.capitalize()} {self.tr('Command', 'Perintah')}\n{self.command.help or ''}"
        app_command = next(
        ...
        if app_command:
            if app_command.options:
                desc += (
                    f"\n\n**{self.tr('Primary Command', 'Perintah Utama')}:** `{prefix}{app_command.name} {params}`"
                )
                desc += f"\n\n**{self.tr('Options', 'Opsi')}:**\n"
                ...
                desc += f"\n\n**{self.tr('Primary Command', 'Perintah Utama')}:** {mention}"
        else:
            desc += f"\n\n**{self.tr('Primary Command', 'Perintah Utama')}:** `{prefix}{self.command.name} {params}`"
            if isinstance(self.command, commands.Group):
                desc += f"\n\n**{self.tr('Subcommands', 'Subperintah')}:**\n"
